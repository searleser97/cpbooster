...
:::important
The key must match with the extension used in the file name.
:::
...
The command that will be used to run `mixed` type languages like (java, kotlin, scala, ...).
...
Specifies whether the language is "compiled", "interpreted" or "mixed".
`cpbooster` is aware that some languages can be "interpreted" as well as "compiled",
therefore with this option it gives you the chance to tell it how to work with
the language you provide.
