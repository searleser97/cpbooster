...
:::tip
By default `cpbooster` looks for the configuration file in `$HOME/cpbooster-config.json` or as second option
in `$HOME/.cpbooster/cpbooster-config.json`. Take a look at [**this issue**](https://github.com/searleser97/cpbooster/issues/7)
for context.
:::
